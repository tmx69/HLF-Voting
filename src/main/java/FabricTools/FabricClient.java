...
public class FabricClient {
    private static final String channelName = "common";
    private static final String chaincodeName = "voting";
    ...
    public FabricClient(String url, String org) throws Exception{
        URL ca = new URL(url);
        ...
        HttpsURLConnection con = (HttpsURLConnection)ca.openConnection();
        con.connect();

        Certificate[] certs = con.getServerCertificates();

        String key = "-----BEGIN CERTIFICATE-----\r\n" + Base64.getEncoder().encodeToString(certs[0].getEncoded()) + "\r\n-----END CERTIFICATE-----\r\n";

        Properties prop = new Properties();
        prop.put("pemBytes", key.getBytes());

        this.caClient = getHfCaClient(url, prop, org);
    }

    public FabricClient(String url, String org, Properties caClientProperties) {
        this.caClient = getHfCaClient(url, caClientProperties, org);
    }
    ...
    public HFCAClient getHfCaClient(String caUrl, Properties caClientProperties, String org) {
        try {
            HFCAClient caClient = HFCAClient.createNewInstance(org, caUrl, caClientProperties);
    ...
    public AppUser enrollUser(String userId, String org, String msp, String secret) throws
            Exception {
        return new AppUser(userId, org, msp, caClient.enroll(userId, secret));
    }
